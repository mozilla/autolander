var bugzilla = require('./bugzilla');
var GitHubApi = require('github');
var thunkify = require('thunkify');
var debug = require('debug')('autolander:github');
var taskgraph = require('./taskgraph');

/**
 * Initializes the github API.
 */
exports.init = function *(config) {
  var githubApi = new GitHubApi({
    version: '3.0.0',
    debug: false,
    protocol: 'https',
    host: config.host,
    //pathPrefix: '/api/v3',
    timeout: 5000
  });

  githubApi.authenticate({
    type: "oauth",
    token: config.githubConfig.token
  });

  return githubApi;
}

exports.COMMENTS = {
  NO_BUG_FOUND: 'Autolander could not find a bug number in your pull request title. All pull requests should be in the format of: Bug [number] - [description].',
  NON_INTEGRABLE: 'The pull request could not be applied to the integration branch. Please try again after current integration is complete.',
  CI_FAILED: 'The pull request failed to pass integration tests. It could not be landed in master. Thank you for playing, please try again.'
};

/**
 * Comments on a pull request.
 * @param {Object} runtime
 * @param {Object} pull
 * @param {String} comment
 */
exports.addComment = function *(runtime, pull, comment) {
  var repoParts = pull.head.repo.full_name.split('/');
  var doComment = thunkify(runtime.githubApi.issues.createComment);
  yield doComment({
    body: comment,
    user: repoParts[0],
    repo: repoParts[1],
    number: pull.number,
    token: runtime.config.githubConfig.token
  });
}

/**
 * Integrates a pull request into the integration branch.
 * @param {Object} runtime
 * @param {Number} bugId
 * @param {Object} pull
 */
exports.integratePullRequest = function * (runtime, bugId, pull) {
  yield exports.maybeCreateIntegrationBranch(runtime, pull);

  var branchName = 'integration-' + pull.base.ref;
  var repoParts = pull.head.repo.full_name.split('/');

  try {
    var getBranch = thunkify(runtime.githubApi.repos.getBranch.bind(runtime.githubApi.repos));
    integrationBranch = yield getBranch({
      user: repoParts[0],
      repo: repoParts[1],
      branch: branchName,
      token: runtime.config.githubConfig.token
    });
    debug('integration branch as parent is:', integrationBranch.commit.sha);

    var getIntegrationTipTree = thunkify(runtime.githubApi.gitdata.getTree.bind(runtime.githubApi.gitdata));
    var integrationTipTree = yield getIntegrationTipTree({
      user: repoParts[0],
      repo: repoParts[1],
      sha: integrationBranch.commit.sha,
      token: runtime.config.githubConfig.token
    });
    debug('integration branch tree is', integrationTipTree);

    var tipCommit = thunkify(runtime.githubApi.gitdata.getCommit.bind(runtime.githubApi.gitdata));
    var integrationTipCommit = yield tipCommit({
      user: repoParts[0],
      repo: repoParts[1],
      sha: integrationTipTree.sha,
      token: runtime.config.githubConfig.token
    });
    debug('DIFFERENCE IS:', integrationTipCommit.tree.sha, integrationTipTree.tree[0].sha);

    var getCommit = thunkify(runtime.githubApi.gitdata.getCommit.bind(runtime.githubApi.gitdata));
    var commit = yield getCommit({
      user: repoParts[0],
      repo: repoParts[1],
      sha: pull.head.sha,
      token: runtime.config.githubConfig.token
    });

    var createTree = thunkify(runtime.githubApi.gitdata.createTree.bind(runtime.githubApi.gitdata));
    var newTree = yield createTree({
      user: repoParts[0],
      repo: repoParts[1],
      tree: [
        // Some mess of files/attributes.
      ],
      base_tree: commit.tree.sha,
      token: runtime.config.githubConfig.token
    });

    var createCommit = thunkify(runtime.githubApi.gitdata.createCommit.bind(runtime.githubApi.gitdata));
    var createdCommit = yield createCommit({
      user: repoParts[0],
      repo: repoParts[1],
      message: commit.message,
      tree: newTree.sha,
      parents: [integrationBranch.commit.sha],
      token: runtime.config.githubConfig.token
    });
    debug('created commit', createdCommit);

    var updateRef = thunkify(runtime.githubApi.gitdata.updateReference.bind(runtime.githubApi.gitdata));
    var ref = yield updateRef({
      user: repoParts[0],
      repo: repoParts[1],
      ref: 'heads/' + branchName,
      sha: createdCommit.sha,
      token: runtime.config.githubConfig.token
    });

    debug('updated integration branch ref', branchName, pull.head.sha);
  } catch(e) {
    debug('could not fast-forward integration branch', bugId, pull.number, pull.head.sha, e);
    yield exports.addComment(runtime, pull, exports.COMMENTS.NON_INTEGRABLE);

    var addComment = thunkify(runtime.bugzillaApi.addComment.bind(runtime.bugzillaApi));
    yield addComment(bugId, {
      comment: pull.html_url + '\n\n' + exports.COMMENTS.NON_INTEGRABLE
    });

    yield bugzilla.removeCheckinNeeded(runtime, bugId);
    return;
  }

  try {
    // Now schedule a taskgraph.
    yield taskgraph.create(runtime, bugId, pull);
  } catch(e) {
    debug('could not create taskgraph', bugId, pull.number);

    // Repos without a taskgraph.json skip the integration step and can land directly in master.
    yield bugzilla.mergePullRequest(runtime, bugId, pull);
  }
};

/**
 * Creates the integration branch if needed.
 * @param {Object} runtime
 * @param {Object} pull
 */
exports.maybeCreateIntegrationBranch = function * (runtime, pull) {
  var branchName = 'integration-' + pull.base.ref;
  var repoParts = pull.head.repo.full_name.split('/');
  debug('trying to create integration branch', branchName);

  var existingBranch;
  var getBranch = thunkify(runtime.githubApi.repos.getBranch.bind(runtime.githubApi.repos));
  try {
    existingBranch = yield getBranch({
      user: repoParts[0],
      repo: repoParts[1],
      branch: branchName,
      token: runtime.config.githubConfig.token
    });
  } catch(e) {
  }

  if (existingBranch) {
    debug('found existing branch', branchName);
  } else {
    debug('creating integration branch', branchName);

    var getRef = thunkify(runtime.githubApi.gitdata.getReference.bind(runtime.githubApi.gitdata));
    var masterRef = yield getRef({
      user: repoParts[0],
      repo: repoParts[1],
      ref: 'heads/' + pull.base.ref,
      token: runtime.config.githubConfig.token
    });
    debug('got master ref', masterRef.object.sha);

    try {
      var createRef = thunkify(runtime.githubApi.gitdata.createReference.bind(runtime.githubApi.gitdata));
      var ref = yield createRef({
        user: repoParts[0],
        repo: repoParts[1],
        ref: 'refs/heads/' + branchName,
        sha: masterRef.object.sha,
        token: runtime.config.githubConfig.token
      });
    } catch(e) {
      debug('integration branch already defined!', branchName);
    }
    debug('created reference', ref.object.sha);
  }
};
