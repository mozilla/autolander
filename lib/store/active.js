...
ActiveStore.prototype.isBugIntegrating = function * (user, repo, bugId) {
  ...
  var integrations = this.getCurrentIntegrations(user, repo, null /* branch */, bugId);
...
/**
 * Gets all curently integrating jobs for a repo and branch.
 ...
 * @param {String} user
 * @param {String} repo
 * @param {String} branch= Filters by branch
 * @param {Number} bugId= Filters by bugId if passed.
 */
ActiveStore.prototype.getCurrentIntegrations = function * (user, repo, branch, bugId) {
  debug('getCurrentIntegrations', user, repo, branch, bugId);
  ...
  try {
    ...
    if (branch) {
      details.query.and('githubBaseBranch', '==', branch);
    }
