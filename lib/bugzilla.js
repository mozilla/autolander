 ...
 */
exports.canLand = function * (runtime, bug, attachment) {
  ...
  if (hasReview && hasSuggestedReview) {
    ...
    // adding checkin-needed to a bug is quite a deliberate action, and if the user wanted to thye
    // could simply clear the R- and request checkin-needed.
    // As long as the author has an R+ from a suggested reviewer, they are good.
    return true;
  } else if (hasReview && bug.keywords.indexOf('checkin-needed') !== -1) {
    // Else if we have a review and checkin-needed, we leave a comment.
    var comment = {
...
exports.attachPullRequest = function * (runtime, bugId, pull) {
  ...
  var attachment = {
    ...
    // Create a summary like: [PullReq] KevinGrandon:my_whatever_branch to mozilla-b2g:master
    summary: '[PullReq] ' + pull.head.label + ' to ' + pull.base.label,
...
exports.getOpenPullsForBug = function * (runtime, bug, filter) {
  ...
  for (var i = 0, iLen = attachments.length; i < iLen; i++) {
    ...
    var passesFilter = yield filter(runtime, bug, attachment);
    if (passesFilter) {
