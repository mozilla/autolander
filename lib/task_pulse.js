...
TaskPulse.prototype = {
  init: function * () {
    ...
    var listener = new PulseListener({
      queueName:  this.config.taskPulseConfig.queueName,
      credentials: {
    ...
    var route = 'route.' + this.config.taskPulseConfig.route + '.#';
    ...
    listener.on('message', co(function * (msg) {
      if (self._handler) {
        try {
          ...
          debug('error processing message', msg, e, e.stack);
        }
      } else {
        debug('handler not ready for message', msg);
      }
    }));
