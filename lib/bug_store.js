 ...
 * XXX: Currently using the in-memory store for this.
 */
AzureApi.prototype.isIntegrating = function *(bugId) {
  debug('isIntegrating bugId', bugId);
  var found = false;
  for (var i in this.activeIntegrations) {
    var integration = this.activeIntegrations[i];
    if (integration.bugId === bugId) {
      found = integration;
      break;
    }
  }
